--- 
tags: []
wordpress_url: http://hplogsdon.com/?p=130
published: true
layout: post
wordpress_id: 130
categories: 
- Uncategorized
author_login: uxp
author_email: uxp@bsdeviant.org
title: Wipe and Reinstall is faster
comments: []
description: |
  I've moved this blog from one domain to another time again, partly because I didn't want to renew that domain name for another year, when I really didn't like it all that much. 

author_url: http://hplogsdon.com
status: publish
date: 2009-09-24 01:13:14 -06:00
author: uxp
---
I've moved this blog from one domain to another time again, partly because I didn't want to renew that domain name for another year, when I really didn't like it all that much. 

Figured I might run this for a while, then see how it goes. I'll probably end up doing something completely different in a few months anyways. I don't care. SEO isn't the reason I'm doing this.

